...
ロボットを動かす際は、**Important**の項目を参照してください。

## Windows 環境
* [環境セットアップ](#環境セットアップ)**Important**
* [コントローラー操作](###補助資料1]コントローラー操作 (参考値))
* [カメラ動作確認](##カメラ動作確認 (Windoes環境において、カメラの動作を確認したいときに使用してください))
...
**[推奨]**もしくは、リポジトリに含まれる`requirements.txt`ファイルを使ってインストールすることも可能です。
```bash
...
で決定  
**→接続が可能となる。**
以下の章において、Raspberry Pi への書き込みはReal VNC経由での書き込みを推奨します。(実働環境でうごかすため)

これで、Windows環境での基本的なセットアップは完了です。  
以下に、コントローラ操作において本コードが用いている対応表をしめします。
### 補助資料1]コントローラー操作 (参考値)
...
(末尾)ソースコードリンク
## カメラ動作確認 (Windoes環境において、カメラの動作を確認したいときに使用してください)
